...
const notifs = ref([
  {
    id: 1,
    text: t("topbar.notifs.newUserSignedUp"),
    time: t("topbar.times.twoMinAgo"),
  },
  {
    id: 2,
    text: t("topbar.notifs.systemBackupComplete"),
    time: t("topbar.times.oneHourAgo"),
  },
  {
    id: 3,
    text: t("topbar.notifs.paymentReceivedShort"),
    time: t("topbar.times.twoHoursAgo"),
  },
]);
...
const {
  ...
  onSuccess,
  onError,
} = useRequest(authApi.logout, { immediate: false });
...
<template>
  <header
    class="fixed top-6 z-10 transition-all duration-500 bg-[rgba(15,17,23,0.65)] backdrop-blur-2xl border border-white/10 rounded-2xl shadow-[0_0_30px_rgba(236,72,153,0.1)] flex items-center justify-between px-6 py-3 bg-gradient-to-br from-[rgba(139,92,246,0.08)] via-[rgba(236,72,153,0.08)] to-[rgba(34,211,238,0.08)] hover:shadow-[0_0_50px_rgba(236,72,153,0.2)]"
    :style="{
  ...
  >
    ...
    <div class="relative flex-1 max-w-xs mx-6 hidden md:flex">
      <Search
        class="w-4 h-4 text-gray-400 absolute top-1/2 -translate-y-1/2"
        :class="isRTL ? 'right-3' : 'left-3'"
      />
      <input
        v-model="search"
        :placeholder="$t('topbar.searchPlaceholder')"
        class="w-full py-1.5 rounded-xl bg-white/5 text-gray-200 border border-white/10 focus:outline-none focus:ring-2 focus:ring-fuchsia-500/40 text-sm"
    ...
    <div class="flex items-center gap-4 relative">
      ...
      <div class="relative" @click.stop="toggleDropdown('profile')">
        ...
        >
          ...
          />
          <span v-else-if="authStore.userInitials" class="text-xs">{{
