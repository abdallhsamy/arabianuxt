...
const send = async (): Promise<void> => {
  ...
  if (!v) return
  messages.value.push({ id: crypto.randomUUID?.() ?? String(Date.now()), from: 'user', text: v, time: 'now' })
  input.value = ''
  await nextTick()
  setTimeout(() => {
    messages.value.push({ id: crypto.randomUUID?.() ?? String(Date.now()), from: 'agent', text: 'Thanks! We are looking into it.', time: 'now' })
  }, 600)
}
</script>
...
<template>
  <div v-if="open" class="fixed inset-0 z-50 bg-black/60 backdrop-blur-md flex items-center justify-center p-4">
    <div class="w-full max-w-2xl rounded-2xl overflow-hidden">
      <div class="p-[2px] bg-[conic-gradient(from_var(--angle),#8B5CF6_0%,#EC4899_60%,#22D3EE_100%)] animate-rotate-gradient rounded-2xl">
        <div class="bg-[rgba(15,17,23,0.96)] border border-white/10 rounded-2xl p-4">
          <div class="flex items-center justify-between mb-3">
            <div class="flex items-center gap-2 text-white font-semibold">
              <MessageSquare class="w-5 h-5 text-cyan-400" />
              {{ subject }} <span class="text-gray-400 text-xs">#{{ ticketId }}</span>
            </div>
            <button class="text-gray-300 hover:text-white" @click="close"><X class="w-5 h-5" /></button>
          </div>
          ...
          <form class="mt-3 flex gap-2" @submit.prevent="send">
            <input v-model="input" class="input-dark flex-1" placeholder="Reply…" />
            <button class="px-3 py-2 rounded-xl text-white bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 hover:brightness-110 transition-all">
              <Send class="w-4 h-4" />
            </button>
          </form>
        </div>
