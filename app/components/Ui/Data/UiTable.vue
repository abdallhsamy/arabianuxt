...
const sorts = ref<{ key: string; dir: SortDir }[]>(
  ...
  if (props.searchable && search.value.trim()) {
    ...
    data = data.filter(r =>
      visibleColumns.value.some(
        c =>
          c.searchable !== false &&
          normalize(getCell(r, c)).toLowerCase().includes(q)
      )
    );
  }
...
const totalRows = computed(() =>
  props.serverMode ? props.total : sortedRows.value.length
);
const pages = computed(() =>
  Math.max(1, Math.ceil(totalRows.value / internalPageSize.value))
);
...
const allOnPageChecked = computed(
  () =>
    ...
    paged.value.every(r => selected.value.has(props.rowKey(r)))
);
...
/* ------------------ Sorting ------------------ */
const nextDir = (d?: SortDir): SortDir | null =>
    ...
    <div class="mb-3 flex flex-wrap items-center gap-2">
      ...
      <div class="ms-auto flex items-center gap-2">
        ...
        <details class="relative">
          ...
          <div
            class="absolute right-0 mt-2 w-56 rounded-xl border border-white/10 bg-black/70 backdrop-blur-xl p-2 shadow-2xl z-10"
          >
            <label
              ...
              <input
                ...
                @change="
                  ($event.target as HTMLInputElement)?.checked
                    ? (columnOrder = [...columnOrder, col.key].filter(
                        (v, i, a) => a.indexOf(v) === i
                      ))
        ...
        <tbody>
          ...
          >
            <td v-if="props.selectable" class="px-3 py-2 align-middle">
              <input
                type="checkbox"
                :checked="selected.has(props.rowKey(row))"
      ...
      <div class="ms-auto flex items-center gap-2">
        ...
        <select
          ...
          @change="
            ...
            $emit('update:pageSize', internalPageSize);
            internalPage = 1;
            $emit('update:page', 1);
          "
        >
          <option v-for="n in props.pageSizes" :key="n" :value="n">
            {{ n }}
          </option>
        </select>
        ...
        <button
          ...
          @click="
            internalPage = Math.max(1, internalPage - 1);
            $emit('update:page', internalPage);
          "
        >
          Prev
        </button>
        <button
          class="rounded-md px-3 py-1 border border-white/10 bg-white/5 text-sm disabled:opacity-50"
          :disabled="internalPage >= pages"
          @click="
            internalPage = Math.min(pages, internalPage + 1);
            $emit('update:page', internalPage);
          "
        >
          Next
