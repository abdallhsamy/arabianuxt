...
const messages = computed(() =>
    selectedUser.value
        ? msgStore.byUser(selectedUser.value, userStore.me.id)
        : []
)
...
<template>
  <!-- Floating Button -->
  <button
      class="fixed z-50 bottom-4 right-4 p-4 rounded-full bg-gradient-to-r from-fuchsia-600 to-cyan-600 text-white shadow-lg hover:scale-105 transition"
      @click="open = !open"
  >
    💬
    <span
  ...
  <transition name="fade">
    <div
        v-if="open"
        class="fixed bottom-20 right-6 w-[380px] max-h-[580px] bg-black/80 border border-white/10 backdrop-blur-xl rounded-2xl shadow-2xl flex flex-col overflow-hidden"
    >
      <!-- Header -->
      ...
      <div v-else class="flex-1 flex flex-col">
        <!-- Back -->
        <div class="flex items-center gap-2 border-b border-white/10 bg-white/5 px-3 py-2">
          <button @click="selectedUser = null" class="text-gray-400 hover:text-white">←</button>
          <img
        ...
        <div class="flex-1 overflow-y-auto p-3 space-y-3">
          ...
          >
            <div
                ...
                :class="m.from === userStore.me.id ? 'bg-fuchsia-600/20' : 'bg-white/5'"
            >
              {{ m.text }}
            </div>
