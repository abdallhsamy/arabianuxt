...
interface Prompt { id: string; role: 'user' | 'assistant'; content: string; ts: number }
...
watch(history, v => localStorage.setItem(key, JSON.stringify(v)), { deep: true })
...
<template>
  <div class="rounded-2xl bg-white/5 border border-white/10 p-4 space-y-2">
    <div class="flex justify-between items-center mb-2">
      ...
      <button class="text-xs text-fuchsia-400 hover:underline" @click="clear">Clear</button>
    </div>
    <div class="max-h-80 overflow-auto text-sm space-y-2">
      ...
      <p v-if="!history.length" class="text-gray-500 text-center text-sm">No prompts yet.</p>
    </div>
  </div>
</template>
