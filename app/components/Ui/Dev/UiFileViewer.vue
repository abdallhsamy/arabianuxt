...
const onCopy = async (): Promise<void> => {
  const text = detectKind.value === 'json'
...
const onDownload = (): void => {
  const blob = detectKind.value === 'json'
      ? new Blob([JSON.stringify(props.json, null, 2)], { type: 'application/json' })
...
<template>
  <div class="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl overflow-hidden">
    <header class="flex items-center justify-between px-4 py-2 border-b border-white/10">
      <div class="text-sm text-gray-300 truncate">{{ filename }}</div>
      <div class="flex items-center gap-2">
        <button class="text-xs px-2 py-1 rounded bg-white/5 hover:bg-white/10 text-gray-200" @click="onCopy">Copy</button>
        <button class="text-xs px-2 py-1 rounded bg-white/5 hover:bg-white/10 text-gray-200" @click="onDownload">Download</button>
      </div>
    </header>

    <!-- JSON -->
    <UiJsonInspector v-if="detectKind==='json'" :value="json" class="max-h-[480px] overflow-auto p-3" />

    <!-- Code/Text -->
    <pre v-else-if="detectKind==='code' || detectKind==='text'" class="p-4 text-sm text-gray-100 overflow-auto"><code :class="lang">{{ code }}</code></pre>
