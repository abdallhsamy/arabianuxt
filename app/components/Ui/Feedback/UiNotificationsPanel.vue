...
const unreadCount = computed<number>(() => props.items.filter(i => !i.read).length)
</script>
      ...
      <ul class="p-3 space-y-2 overflow-y-auto flex-1">
        ...
        >
          <div class="flex items-start justify-between">
            ...
            <div v-if="n.time" class="text-[11px] text-gray-500">{{ n.time }}</div>
          </div>
          <div v-if="n.body" class="text-sm text-gray-300 mt-1">{{ n.body }}</div>
        </li>
