...
import type { UiButtonProps } from "./UiButton.type";
import {
  UiButtonVariants,
  UiButtonSizes,
  UiButtonIconPositions,
} from "./UiButton.type";

const props = withDefaults(defineProps<UiButtonProps>(), {
  variant: UiButtonVariants.Primary,
  size: UiButtonSizes.Medium,
  iconPosition: UiButtonIconPositions.Left,
  loading: false,
  disabled: false,
  full: false,
  iconOnly: false,
...
const base =
  "inline-flex items-center justify-center font-semibold rounded-xl transition-all select-none outline-none focus-visible:ring-2 ring-offset-2 ring-fuchsia-500/50 active:scale-[0.98]";
...
<template>
  ...
  >
    ...
    <template v-else>
      <component
        v-if="props.icon && props.iconPosition === UiButtonIconPositions.Left"
        :is="props.icon"
        class="w-4 h-4"
      />
      <span v-if="props.label">{{ props.label }}</span>
      <component
        v-if="props.icon && props.iconPosition === UiButtonIconPositions.Right"
        :is="props.icon"
        class="w-4 h-4"
      />
    </template>
