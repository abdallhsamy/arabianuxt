...
<template>
  <div>
    <button
      class="fixed bottom-6 ltr:right-6 rtl:left-6 rounded-full bg-gradient-to-r from-fuchsia-600 to-cyan-600 text-white w-12 h-12 flex items-center justify-center shadow-lg"
      @click="open = !open"
    >
      💬
    </button>
    ...
    <transition name="fade">
      <div
        v-if="open"
        class="fixed bottom-20 ltr:right-6 rtl:left-6 w-80 bg-white/10 backdrop-blur-xl border border-white/20 rounded-xl p-4 flex flex-col h-96"
      >
        <div class="flex-1 overflow-y-auto space-y-3 mb-3">
          <div
            ...
            :class="
              m.from === AssistChatMessageFrom.User
                ? 'bg-fuchsia-600/40 self-end text-right'
