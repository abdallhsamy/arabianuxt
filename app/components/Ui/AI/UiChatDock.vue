...
const send = () => {
  ...
  messages.value.push({ id: crypto.randomUUID(), role: 'user', content: input.value })
      ...
      >
        ...
        <div class="flex-1 p-3 space-y-2 overflow-y-auto max-h-96">
          <div
              ...
              :class="m.role === 'user' ? 'bg-fuchsia-600/30 self-end text-right ml-auto' : 'bg-white/10 text-gray-100'"
          >
            {{ m.content }}
          </div>
        </div>
        ...
        <div class="border-t border-white/10 flex items-center p-2">
          <input
              ...
              @keydown.enter="send"
          />
          <button class="px-3 text-fuchsia-400 hover:text-fuchsia-300" @click="send">↩</button>
        </div>
      </div>
    </transition>
  </div>
