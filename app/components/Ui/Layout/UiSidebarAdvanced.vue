...
const baseCls =
  "fixed z-40 top-4 left-4 h-[calc(100vh-2rem)] rounded-2xl bg-black/50 backdrop-blur-xl border border-white/10 shadow-2xl";
...
<template>
  <aside
    :class="[baseCls, wCls, 'flex flex-col']"
    role="navigation"
    aria-label="Sidebar"
  >
    <!-- Header -->
    ...
    >
      <div class="flex items-center gap-2">
        <div
          class="h-8 w-8 rounded-lg bg-gradient-to-br from-fuchsia-500/40 to-cyan-500/40"
        />
        <span v-if="isExpanded" class="text-gray-100 font-semibold"
          >Dashboard</span
        >
      </div>
      <button
        class="rounded-lg border border-white/10 bg-white/5 text-gray-200 px-2 py-1"
        @click="toggle"
      >
        {{ isExpanded ? "⟨" : "⟩" }}
      </button>
          ...
          <div v-else class="px-1">
            ...
            >
              ...
              <ChevronDown
                ...
                :class="{ 'rotate-180': openMap[it.label] }"
              />
            </button>
            <transition name="fade">
              ...
              >
                <li v-for="(ch, j) in it.children" :key="j">
                  <NuxtLink
                    :to="ch.to || '#'"
                    class="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-white/10 text-gray-300"
                  >
                    <component
                      :is="renderIcon(ch.icon)"
                      class="w-3.5 h-3.5 text-gray-500"
                    />
                    <span class="truncate">{{ ch.label }}</span>
                    <span
                      v-if="ch.badge"
                      class="ms-auto text-[10px] rounded bg-white/10 px-1.5 border border-white/10"
                      >{{ ch.badge }}</span
                    >
                  </NuxtLink>
                </li>
    ...
    <div class="mt-auto p-2 border-t border-white/10">
      <NuxtLink
        to="/upgrade"
        class="group flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-white/10"
      >
