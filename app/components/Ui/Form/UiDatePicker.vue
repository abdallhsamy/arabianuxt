...
import { CalendarDays, ChevronLeft, ChevronRight, Globe2 } from 'lucide-vue-next'
...
const monthName = computed(() => {
  ...
  return currentView.value.toLocaleString('default', { month: 'long', year: 'numeric' })
})
...
const monthDays = computed<Cell[]>(() => {
  if (isHijri.value) {
    ...
    const startH = base.clone().iYear(hYear).iMonth(hMonth).iDate(1).startOf('day')
...
<template>
  <div ref="pickerRef" class="relative w-full">
    ...
    <button
        ...
        :class="[s, props.disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:bg-white/10']"
    ...
    <transition name="fade">
      ...
      >
        ...
        <div class="flex items-center justify-between mb-2">
          <button class="p-1 rounded-md bg-white/10 hover:bg-white/15" @click.stop="prevMonth">
            <ChevronLeft class="w-4 h-4 text-gray-300" />
          </button>
          <div class="text-gray-100 text-sm font-medium">{{ monthName }}</div>
          <button class="p-1 rounded-md bg-white/10 hover:bg-white/15" @click.stop="nextMonth">
        ...
        <div class="flex justify-between items-center mt-3 text-xs text-gray-400">
          ...
          <button
              ...
              @click.stop="selected = null; emit('update:modelValue', null); emit('update:hijri', null)"
          >
            Clear
          </button>
        </div>
