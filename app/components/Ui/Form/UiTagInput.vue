...
import { ref } from 'vue'
export interface UiTagInputProps { modelValue: string[]; placeholder?: string }
const props = withDefaults(defineProps<UiTagInputProps>(), { placeholder: 'Add tag…' })
const emit = defineEmits<{ (e:'update:modelValue', v: string[]): void }>()
const text = ref('')
const add = (): void => {
  const v = text.value.trim(); if (!v) return
  const set = Array.from(new Set([...props.modelValue, v]))
  emit('update:modelValue', set); text.value = ''
}
const remove = (tag: string): void => emit('update:modelValue', props.modelValue.filter(t => t !== tag))
</script>
