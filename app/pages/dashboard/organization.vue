    ...
    <GradientCard>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
          <input v-model="orgName" class="input-dark" :placeholder="t('pages.organization.orgNamePlaceholder')" />
          <input v-model="domain" class="input-dark" :placeholder="t('pages.organization.domainPlaceholder')" />
          <select v-model="plan" class="input-dark">
            <option>{{ t('pages.organization.plans.free') }}</option><option>{{ t('pages.organization.plans.pro') }}</option><option>{{ t('pages.organization.plans.enterprise') }}</option>
          </select>
          <div class="flex items-center gap-3">
            <input v-model="primaryColor" type="color" class="h-10 w-14 rounded-lg border border-white/10 bg-transparent" />
            <span class="text-sm text-gray-300">{{ t('pages.organization.primaryColor') }}</span>
          </div>
        </div>
        ...
        <div class="flex flex-col items-center justify-center gap-3">
          <div class="w-28 h-28 rounded-xl overflow-hidden bg-white/10 border border-white/10 flex items-center justify-center">
            ...
            <span v-else class="text-gray-400 text-xs">{{ t('pages.organization.noLogo') }}</span>
          </div>
          <label class="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white text-sm cursor-pointer">
            {{ t('pages.organization.uploadLogo') }} <input type="file" accept="image/*" class="hidden" @change="onLogo" />
          </label>
        </div>
      </div>
