    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-1 p-[2px] rounded-2xl bg-[conic-gradient(from_var(--angle),#8B5CF6_0%,#22D3EE_60%,#EC4899_100%)] animate-rotate-gradient">
        <div class="rounded-2xl bg-[rgba(15,17,23,0.92)] backdrop-blur-xl border border-white/10 p-6">
          <h2 class="text-white font-semibold text-lg mb-3 flex items-center gap-2">
            <CreditCard class="w-5 h-5 text-cyan-400" /> Payment Method
          </h2>
          <p class="text-gray-300 text-sm">Card on file</p>
          <p class="text-white font-medium mt-1">{{ maskCard(cardLast4) }}</p>
          <div class="mt-4 flex gap-3">
            <button class="px-4 py-2 rounded-xl bg-gray-700 text-white hover:bg-gray-600 transition-all" @click="addCardOpen = true">Update Card</button>
            <button class="px-4 py-2 rounded-xl text-white bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 hover:brightness-110 transition-all">Pay Now {{ currency(nextAmount) }}</button>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2 p-[2px] rounded-2xl bg-[conic-gradient(from_var(--angle),#22D3EE_0%,#8B5CF6_60%,#EC4899_100%)] animate-rotate-gradient">
        <div class="rounded-2xl bg-[rgba(15,17,23,0.92)] backdrop-blur-xl border border-white/10 p-6 overflow-x-auto">
          <h2 class="text-white font-semibold text-lg mb-4 flex items-center gap-2"><FileText class="w-5 h-5 text-fuchsia-400" /> Invoices</h2>
    ...
    <Modal v-model:open="addCardOpen" title="Update Payment Method" confirm-text="Save Card">
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-3">
          <input class="input-dark w-full" placeholder="MM/YY" />
