...
/* ------------------ Fake data ------------------ */
const roles = [
  t("pages.componentTable.roles.admin"),
  t("pages.componentTable.roles.editor"),
  t("pages.componentTable.roles.viewer"),
] as const;
...
const cols = ref<TableColumn<User>[]>([
  {
    key: "name",
    header: t("pages.componentTable.headers.name"),
    sortable: true,
    filter: { type: "text" },
  },
  {
    key: "email",
    header: t("pages.componentTable.headers.email"),
    sortable: true,
    filter: { type: "text" },
  },
  {
  ...
  {
    key: "usage",
    header: t("pages.componentTable.headers.usage"),
    sortable: true,
    filter: { type: "range" },
  },
  {
    ...
    >
      ...
      <template #empty>
        <div class="py-6 text-gray-400">
          {{ t("pages.componentTable.emptyState") }}
        </div>
      </template>
    </UiTable>

    <div class="text-sm text-gray-400">
      {{ t("pages.componentTable.selectedRows") }}:
      {{ selected.join(", ") || t("pages.componentTable.none") }}
    </div>
  </section>
</template>
