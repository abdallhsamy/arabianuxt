...
const messages = ref<Msg[]>([
  { id: 'm1', role: 'assistant', text: 'Hey! I can help summarize analytics, generate reports, and draft emails.' }
])
...
const send = async (): Promise<void> => {
  ...
  setTimeout(() => {
    messages.value.push({ id: crypto.randomUUID?.() ?? String(Date.now()), role: 'assistant', text: `Got it. "${val}" — generating a concise response…` })
  }, 600)
}
</script>
...
<template>
  <section>
    <h1 class="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-fuchsia-400 via-indigo-400 to-cyan-400 mb-6">AI Assistant</h1>
      ...
      <transition name="fade">
        <div v-if="open" class="fixed right-6 bottom-20 z-40 w-[360px] max-h-[70vh] flex flex-col p-[2px] rounded-2xl bg-[conic-gradient(from_var(--angle),#8B5CF6_0%,#EC4899_60%,#22D3EE_100%)] animate-rotate-gradient">
          <div class="rounded-2xl bg-[rgba(15,17,23,0.96)] backdrop-blur-2xl border border-white/10 p-4 flex flex-col min-h-[320px] max-h-[70vh]">
            <div class="flex items-center gap-2 mb-3 text-white font-semibold"><Bot class="w-5 h-5 text-cyan-400" /> Assistant</div>
            ...
            <form class="mt-3 flex gap-2" @submit.prevent="send">
              <input v-model="input" class="input-dark flex-1" placeholder="Ask anything…" />
              <button class="px-3 py-2 rounded-xl text-white bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 hover:brightness-110 transition-all" title="Send">
                <Send class="w-4 h-4" />
              </button>
            </form>
          </div>
