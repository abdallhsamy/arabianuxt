...
const send = async (): Promise<void> => {
  ...
  setTimeout(() => {
    messages.value.push({
      id: crypto.randomUUID?.() ?? String(Date.now()),
      role: "assistant",
      text: t("pages.assistant.responseMessage", { query: val }),
    });
  }, 600);
};
</script>
  ...
  <section>
    ...
    <div class="relative">
      ...
      <transition name="fade">
        <div
          v-if="open"
          class="fixed ltr:right-6 rtl:left-6 bottom-34 sm:bottom-20 z-40 w-[360px] max-h-[70vh] flex flex-col p-[2px] rounded-2xl bg-[conic-gradient(from_var(--angle),#8B5CF6_0%,#EC4899_60%,#22D3EE_100%)] animate-rotate-gradient"
        >
          <div
            class="rounded-2xl bg-[rgba(15,17,23,0.96)] backdrop-blur-2xl border border-white/10 p-4 flex flex-col min-h-[320px] max-h-[70vh]"
          >
