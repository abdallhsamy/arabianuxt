...
const notification = ref<Notification[]>([
  {
    id: "n1",
    type: "system",
    text: "New dashboard update v1.4 deployed.",
    read: false,
    time: "2m ago",
  },
  {
    id: "n2",
    type: "billing",
    text: "Invoice INV-2025-0042 paid.",
    read: true,
    time: "1d ago",
  },
  {
...
const filtered = computed(() =>
  filter.value === "all"
    ? notification.value
    : notification.value.filter(n => n.type === filter.value)
);
...
<template>
  <section class="space-y-8">
    <div class="flex items-center justify-between">
      ...
      <button class="text-sm text-cyan-300 hover:text-white" @click="markAll">
    ...
    <div class="flex gap-3 text-sm">
      <button
        v-for="type in ['all', 'system', 'billing', 'security']"
        :key="type"
        class="px-3 py-1.5 rounded-xl capitalize"
        :class="
          type === filter
      ...
      >
        {{ t(`pages.notifications.filters.${type}`) }}
      </button>
    </div>

    <div class="space-y-3">
      <NotificationItem v-for="n in filtered" :key="n.id" :n="n" />
      <p v-if="!filtered.length" class="text-gray-400 text-sm text-center py-6">
