...
const prompts = ref<Prompt[]>([
  {
    ...
    text: "Summarize KPIs for this week and produce 3 insights + 3 actions.",
    tags: ["analytics", "general"],
  },
  {
    id: "p2",
    title: "Security incident brief",
    text: "Draft a concise brief for latest incidents including scope, impact, and mitigation.",
    tags: ["security"],
  },
  {
    id: "p3",
    title: "Customer email reply",
    text: "Write a friendly, concise reply acknowledging customer issue and proposing fix steps.",
    tags: ["email", "general"],
  },
]);
...
const filtered = computed<Prompt[]>(() => {
  ...
  return prompts.value.filter(p => {
    const qOk = [p.title, p.text, ...p.tags].some(v =>
      v.toLowerCase().includes(q)
    );
    const tOk =
      !tagsFilter.value.length ||
      tagsFilter.value.every(t => p.tags.includes(t));
    return qOk && tOk;
  });
});
...
const usePrompt = (id: string): void =>
  alert(t("pages.promptLibrary.usePrompt", { id }));
const toggleTag = (arr: string[], tag: string): void => {
  const idx = arr.indexOf(tag);
  idx >= 0 ? arr.splice(idx, 1) : arr.push(tag);
};
</script>
  ...
  <section class="space-y-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      >
        ...
        >
          <h3 class="text-white font-semibold mb-2">
            {{ t("pages.promptLibrary.filters") }}
          </h3>
          <input
            v-model="search"
            class="input-dark w-full mb-3"
            :placeholder="t('pages.promptLibrary.searchPlaceholder')"
          />
          <div class="space-y-2">
            <p class="text-xs text-gray-400">
              {{ t("pages.promptLibrary.tags") }}
            </p>
            <div class="flex flex-wrap gap-2">
              <button
                ...
                :class="
                  tagsFilter.includes(t)
                    ? 'bg-gradient-to-r from-indigo-500/40 to-fuchsia-500/40 text-white border-white/20'
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <!-- Create -->
        <div
          class="p-[2px] rounded-2xl bg-[conic-gradient(from_var(--angle),#22D3EE_0%,#8B5CF6_55%,#EC4899_100%)] animate-rotate-gradient"
        >
          <div
            class="rounded-2xl bg-[rgba(15,17,23,0.94)] border border-white/10 p-5"
          >
            <h3 class="text-white font-semibold mb-3">
              {{ t("pages.promptLibrary.addPrompt") }}
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input
                v-model="newTitle"
                class="input-dark"
                :placeholder="t('pages.promptLibrary.titlePlaceholder')"
              />
              <div>
              ...
              <textarea
                v-model="newText"
                class="input-dark md:col-span-2 min-h-[120px]"
                :placeholder="t('pages.promptLibrary.promptTextPlaceholder')"
              ></textarea>
            </div>
