...
const selectedDate = ref<string>(
  new Date(year, month, today.getDate()).toISOString().slice(0, 10)
);
const events = ref<EventItem[]>([
  {
    id: "e1",
    date: new Date(year, month, 5).toISOString().slice(0, 10),
    title: t("pages.calendar.events.deploy"),
  },
  {
    id: "e2",
    date: new Date(year, month, 12).toISOString().slice(0, 10),
    title: t("pages.calendar.events.securityAudit"),
  },
  {
    id: "e3",
    date: new Date(year, month, today.getDate()).toISOString().slice(0, 10),
    title: t("pages.calendar.events.teamStandup"),
  },
]);
...
<template>
  <section class="space-y-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <!-- Calendar -->
      <div
        class="lg:col-span-2 p-[2px] rounded-2xl bg-[conic-gradient(from_var(--angle),#8B5CF6_0%,#22D3EE_60%,#EC4899_100%)] animate-rotate-gradient"
      >
        <div
            ...
            <button
              ...
              :class="[
                ...
                cell.iso === selectedDate
                  ? 'bg-gradient-to-r from-indigo-500/30 to-fuchsia-500/30 text-white ring-1 ring-white/20'
                  : 'text-gray-300',
              ]"
            >
              {{ cell.label }}
      ...
      >
        ...
        >
          <h3 class="text-white font-semibold mb-3">
            {{ t("pages.calendar.eventsTitle") }} — {{ selectedDate }}
          </h3>
          <ul class="space-y-2">
            <li
              v-for="e in dayEvents"
              :key="e.id"
              class="p-2 bg-white/5 rounded-lg text-sm text-white"
            >
              {{ e.title }}
            </li>
            <li v-if="!dayEvents.length" class="text-sm text-gray-400">
          ...
          <div class="mt-4 flex gap-2">
            <input
              ...
              :placeholder="t('pages.calendar.newEventPlaceholder')"
            />
            <button
              class="px-3 py-2 rounded-xl text-white bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 hover:brightness-110 transition-all flex items-center gap-1"
              @click="addEvent"
            >
