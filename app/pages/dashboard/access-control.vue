...
const headers = [
  ...
  {
    key: "Member",
    label: t("pages.accessControl.member"),
    class: "text-center",
  },
];
      ...
      <DataTable :headers="headers" :rows="rows" row-key="module">
        <template #cell:module="{ row }">
          <span class="capitalize text-white">{{
            t(`pages.accessControl.modules.${row.module}`)
          }}</span>
        </template>
        ...
        <template #cell:Owner="{ row }">
          <div class="flex justify-center">
            <input
              ...
              :checked="(row as PermRow).Owner"
              @change="toggle(row as PermRow, 'Owner')"
            />
          </div>
        </template>
        ...
        <template #cell:Admin="{ row }">
          <div class="flex justify-center">
            <input
              ...
              :checked="(row as PermRow).Admin"
              @change="toggle(row as PermRow, 'Admin')"
            />
          </div>
        </template>
        ...
        <template #cell:Member="{ row }">
          <div class="flex justify-center">
            <input
              ...
              :checked="(row as PermRow).Member"
              @change="toggle(row as PermRow, 'Member')"
            />
          </div>
        </template>
      </DataTable>
