...
const tickets = ref<Ticket[]>([
  ...
  {
    id: "T-1039",
    subject: "Can’t connect GitHub integration",
    status: "pending",
    priority: "medium",
  ...
  {
    id: "T-1033",
    subject: "Feature request: SSO SAML",
    status: "closed",
    priority: "low",
...
const headers = [
  ...
  {
    key: "createdAt",
    label: t("pages.support.tableHeaders.created"),
    class: "text-right",
  },
];
...
<template>
  <section class="space-y-8">
    ...
    <GradientCard>
      ...
      <DataTable :headers="headers" :rows="filtered" row-key="id">
        <template #cell:subject="{ row }">
          <button
            class="text-left text-white hover:underline"
            @click="openTicket(row.id as string)"
          >
            {{ row.subject }}
          </button>
        </template>
        ...
        <template #cell:status="{ row }">
          <span
            ...
            :class="badgeStatus(row.status as any)"
            >{{ row.status }}</span
          >
        </template>
        ...
        <template #cell:priority="{ row }">
          <span
            class="text-xs px-2 py-1 rounded-full"
            :class="badgePriority(row.priority as any)"
            >{{ row.priority }}</span
          >
        </template>
    ...
    <TicketModal
      ...
      :subject="
        filtered.find(t => t.id === activeTicketId)?.subject ??
        t('pages.support.ticket')
      "
    />
  </section>
</template>
