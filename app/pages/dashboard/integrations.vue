...
const confirmRegen = (): void => {
  regenOpen.value = true;
};
// doRegen function removed as it was unused
  ...
  <section class="space-y-8">
    ...
    >
      ...
      >
        ...
        >
          ...
          {{ t("pages.integrations.apiKey") }}
        </h2>
        <div class="flex items-center gap-3">
          <input
            :value="showKey ? apiKey : 'sk-live-********-****'"
            readonly
            class="input-dark w-full"
          />
          <button
            class="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white"
            @click="showKey = !showKey"
          >
            {{
              showKey
                ? t("pages.integrations.hide")
                : t("pages.integrations.show")
            }}
          </button>
          <button
            class="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white flex items-center gap-2"
            @click="copyKey"
          >
            <Check class="w-4 h-4" /> {{ t("pages.integrations.copy") }}
          </button>
          <button
            class="px-3 py-2 rounded-xl text-white bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 hover:brightness-110 flex items-center gap-2"
            @click="confirmRegen"
          >
            <RefreshCw class="w-4 h-4" />
            {{ t("pages.integrations.regenerate") }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div
        ...
        :key="p.name"
        class="p-[2px] rounded-2xl bg-[conic-gradient(from_var(--angle),#22D3EE_0%,#8B5CF6_60%,#EC4899_100%)] animate-rotate-gradient"
      >
        <div
          class="rounded-2xl bg-[rgba(15,17,23,0.92)] backdrop-blur-xl border border-white/10 p-6 flex items-center justify-between"
        >
          <div class="flex items-center gap-3">
          ...
          <button
            ...
            :class="
              p.connected
                ...
                : 'bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 hover:brightness-110'
            "
          >
    ...
    >
      ...
      >
        ...
        >
          ...
          {{ t("pages.integrations.webhooks") }}
        </h2>
        <table class="w-full text-sm">
          <thead class="text-gray-400">
            <tr class="text-left">
              <th class="py-2">
                {{ t("pages.integrations.webhookTableHeaders.url") }}
              </th>
              <th class="py-2">
                {{ t("pages.integrations.webhookTableHeaders.secret") }}
              </th>
              <th class="py-2">
                {{ t("pages.integrations.webhookTableHeaders.status") }}
              </th>
              <th class="py-2 text-right">
                {{ t("pages.integrations.webhookTableHeaders.actions") }}
              </th>
            </tr>
          </thead>
          <tbody class="divide-y divide-white/10">
            ...
            >
                ...
                <span
                  ...
                  :class="
                    w.status === 'active'
                      ...
                      : 'bg-gray-600/30 text-gray-300 border-gray-400/30'
                  "
                >
                  {{ w.status }}
                </span>
              ...
              <td class="py-3 text-right">
                <button
                  class="px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/15 text-white"
                  @click="testPing(w)"
                >
                  {{ t("pages.integrations.sendTest") }}
                </button>
              </td>
            </tr>
          </tbody>
    ...
    <UiModal
      v-model="regenOpen"
      :title="t('pages.integrations.regenerateApiKey')"
      :confirm-text="t('pages.integrations.regenerateButton')"
    >
