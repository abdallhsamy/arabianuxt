...
const logs = ref<Log[]>([
  ...
  {
    id: "3",
    time: "2025-10-07 20:05",
    actor: "mariam",
    action: "paid invoice",
    target: "INV-2025-0041",
    severity: "info",
  },
  {
    id: "4",
    time: "2025-10-07 14:44",
    actor: "system",
...
const headers = [
  ...
  {
    key: "severity",
    label: t("pages.auditLogs.tableHeaders.severity"),
    class: "text-right",
  },
];
...
const filtered = computed<Log[]>(() => {
  ...
  return logs.value.filter(l => {
    ...
    const qOk = [l.actor, l.action, l.target].some(v =>
...
const exportCsv = (): void => {
  const rows = [
    ["time", "actor", "action", "target", "severity"],
...
<template>
  <section class="space-y-8">
    <div class="flex items-center justify-between">
      ...
      <button
        class="px-4 py-2 rounded-xl text-white bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 hover:brightness-110 font-semibold"
        @click="exportCsv"
      >
      ...
      <div class="flex flex-col md:flex-row gap-3 mb-4">
        ...
        <div class="flex gap-2">
          <button
            ...
            :class="
              severity === s
                ? 'bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 text-white'
        ...
        <template #cell:severity="{ row }">
          <div class="text-right">
            <span
              ...
              :class="badge(row.severity as any)"
              >{{ t(`pages.auditLogs.severities.${row.severity}`) }}</span
            >
          </div>
        </template>
      </DataTable>
