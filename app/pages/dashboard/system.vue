...
<template>
  <section class="space-y-8">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      >
        ...
        >
          ...
          >
            ...
            {{ t("pages.system.environment") }}
          </h2>
          <input
            v-model="envName"
            class="input-dark w-full"
            :placeholder="t('pages.system.environmentName')"
          />
        </div>
      </div>
      ...
      >
        ...
        >
          <h2
            class="text-white font-semibold text-lg mb-4 flex items-center gap-2"
          >
            <Sun class="w-5 h-5 text-amber-300" /> {{ t("pages.system.theme") }}
          </h2>
          <div class="flex items-center gap-3">
            <button
              class="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white flex items-center gap-2"
              @click="theme = 'dark'"
            >
              <Moon class="w-4 h-4" /> {{ t("pages.system.dark") }}
            </button>
            <button
              class="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/15 text-white flex items-center gap-2"
              @click="theme = 'light'"
            >
              <Sun class="w-4 h-4" /> {{ t("pages.system.light") }}
            </button>
            <span class="text-gray-400 text-sm"
              >{{ t("pages.system.selected") }} {{ theme }}</span
            >
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          <h2
            class="text-white font-semibold text-lg mb-4 flex items-center gap-2"
          >
            <Timer class="w-5 h-5 text-indigo-400" />
            {{ t("pages.system.sessionTimeout") }}
          </h2>
          <div class="flex items-center gap-3">
            <input
      ...
      <!-- Maintenance -->
      <div
        class="p-[2px] rounded-2xl bg-[conic-gradient(from_var(--angle),#22D3EE_0%,#EC4899_50%,#8B5CF6_100%)] animate-rotate-gradient"
      >
        <div
          class="rounded-2xl bg-[rgba(15,17,23,0.92)] backdrop-blur-xl border border-white/10 p-6"
        >
          <h2
            class="text-white font-semibold text-lg mb-4 flex items-center gap-2"
          >
            <Wrench class="w-5 h-5 text-rose-400" />
            {{ t("pages.system.maintenanceMode") }}
          </h2>
          ...
          <span class="ms-3 text-sm text-white">{{
            maintenance ? t("pages.system.enabled") : t("pages.system.disabled")
          }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="text-right">
      <button
        class="px-6 py-2 rounded-xl text-white bg-gradient-to-r from-indigo-500 via-fuchsia-500 to-cyan-400 hover:brightness-110 transition-all font-semibold"
        @click="saveAll"
      >
