...
<template>
  <div
    class="flex h-[calc(100vh-5rem)] bg-gradient-to-br from-gray-950 via-black to-gray-900 text-gray-100"
  >
    <!-- Sidebar -->
    ...
    <aside
      v-if="selectedMail"
      class="w-[480px] border-l border-white/10 bg-black/50 backdrop-blur-xl flex flex-col"
    >
      <header
        class="flex items-center justify-between p-4 border-b border-white/10"
      >
        <h3 class="font-semibold text-fuchsia-300 truncate">
      ...
      <div class="p-4 flex-1 overflow-y-auto space-y-3">
        <div class="text-sm text-gray-400">
          {{ t("pages.mail.from") }} {{ selectedMail.from }}
        </div>
        <p class="whitespace-pre-wrap text-gray-100">
      ...
      <footer class="flex justify-end gap-2 p-3 border-t border-white/10">
        <button
          class="px-3 py-1.5 bg-white/10 rounded-md text-sm hover:bg-white/20"
        >
          <Archive class="w-4 h-4 inline-block me-1" />
          {{ t("pages.mail.archive") }}
        </button>
        <button
          class="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white rounded-md text-sm"
        >
          <Trash2 class="w-4 h-4 inline-block me-1" />
          {{ t("pages.mail.delete") }}
        </button>
      </footer>
    </aside>
    ...
    <transition name="fade">
      ...
      >
        <div
          class="bg-black/80 rounded-xl border border-white/10 w-full max-w-lg p-6 space-y-4"
        >
          <div class="flex justify-between items-center">
            <h3 class="text-lg font-semibold text-gray-100">
              {{ t("pages.mail.newMessage") }}
            </h3>
            <button class="hover:text-rose-400" @click="showCompose = false">
              <X class="w-5 h-5" />
            </button>
          </div>
          ...
          <div class="flex justify-between items-center">
            <label
              class="flex items-center gap-1 cursor-pointer text-sm text-fuchsia-400 hover:text-fuchsia-300"
            >
              ...
              <input
                ...
                class="hidden"
                @change="
                  newMail.attachments = Array.from(
